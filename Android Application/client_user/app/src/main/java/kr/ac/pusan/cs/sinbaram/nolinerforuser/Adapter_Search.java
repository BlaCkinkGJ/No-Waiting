...
public class Adapter_Search extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        /*mRef.child("Obj_Line List").child(list.get(position)).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                int count=0;
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                    Obj_Line line = (Obj_Line)snapshot.child("INFO").getValue(Obj_Line.class);
                    count++;
                }
