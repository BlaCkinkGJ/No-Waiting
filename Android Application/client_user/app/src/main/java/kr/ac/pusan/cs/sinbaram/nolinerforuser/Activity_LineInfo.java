...
public class Activity_LineInfo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRef.child("Line List").child(Public_ID).child(line.Line_Name).child("INFO").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                line = dataSnapshot.getValue(Obj_Line.class);
        ...
        mRef.child("Line List").child(Public_ID).child(line.Line_Name).child("USER LIST").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                    Obj_User tmp = new Obj_User();
                    tmp = snapshot.getValue(Obj_User.class);
        ...
        RegistBtn.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View view) {
                ...
                if(DB.get(line.Line_Name,2)==null){
                    ...
                    mRef.child("Obj_Line List").child(Public_ID).child(line.Line_Name).child("INFO").setValue(line);
                    mRef.child("Obj_Line List").child(Public_ID).child(line.Line_Name).child("USER LIST").push().setValue(user);
                    Intent intent1 = new Intent(Activity_LineInfo.this,Activity_QRactivity.class);
    ...
    protected void onResume() {
        super.onResume();
        mRef.child("Obj_Line List").child(Public_ID).child(line.Line_Name).child("INFO").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                line = dataSnapshot.getValue(Obj_Line.class);
                EnrollNum.setText(String.valueOf(line.Current_Enrollment_State)+" / "+String.valueOf(line.Max_Number));
        ...
        mRef.child("Obj_Line List").child(Public_ID).child(line.Line_Name).child("USER LIST").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                int count=0;
                for(DataSnapshot snapshot : dataSnapshot.getChildren()){
